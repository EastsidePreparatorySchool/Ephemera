...
import static org.eastsideprep.spacecritters.gamelogic.GridDisk.disk;
...
public class SortedGridDisk extends GridDisk {
    ...
    public SortedGridDisk(Position center, int radius) {
        ...
        if (disk.get((double) radius) == null) {
            createDisk(radius);
        }
        if (!sortedDisks.contains(radius)) {
            sortDisk(radius);
        }
    ...
    void defineShells() {
        ...
        for (IntegerPosition p : specificDisk) {
            ...
            if (rP.x > rLP.x || rP.y > rLP.y) {
                shellTerminaters.add(lp.toString());
            }
