...
//            e.printStackTrace(System.out);
    ...
    public void addAllCustomAliens() {
        if (Constants.searchProjectForAliens) {
            String parent = engine.gamePath;
            System.out.println("SC: searching project folder: " + parent);

            addCustomAliens(parent, "");
        } else {
            System.out.println("SC: searching alien folder: " + engine.alienPath);
    ...
    public void addCustomAliensInJAR(String domain, File f) {
        String packageName;
        String className;

        if (f.getName().toLowerCase().equals("spacecritters.jar")
                || f.getName().toLowerCase().equals("alieninterfaces.jar")) {
            return;
        }
        System.out.println("SG: adding all aliens in jar " + f.getName());
            ...
            for (ZipEntry entry = zip.getNextEntry(); entry != null; entry = zip.getNextEntry()) {
                if (!entry.isDirectory() && (entry.getName().toLowerCase().endsWith("alien.class")
                    ...
                    if (entry.getName().lastIndexOf('/') != -1) {
                        // named package
                        packageName = entry.getName().substring(0, entry.getName().lastIndexOf('/')).replace("/", ".");
