...
public class MainApp implements SparkApplication {
    ...
    public static void main(String[] args) {
        ...
        boolean desktoponly = false,
                serveronly = false;
        if (args.length != 0) {
            desktoponly = args[0].equalsIgnoreCase("desktoponly");
            serveronly = args[0].equalsIgnoreCase("serveronly");
        }
        
        
        if (args.length == 0 || !desktoponly) {
            MainApp.app = new MainApp();

            if (!(args.length == 0 || serveronly)) {
        ...
        } else if (args.length > 0 && (desktoponly || args[0].equalsIgnoreCase("both")) ) {
